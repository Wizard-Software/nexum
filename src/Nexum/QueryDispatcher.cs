...
public sealed class QueryDispatcher : IQueryDispatcher, IInterceptableDispatcher
{
    ...
    /// <summary>
    /// Cache indicating whether any query exception handlers are registered for a given query type.
    /// When false, the runtime fast path can skip try/catch wrapping entirely (zero-alloc path).
    /// DI container is immutable after build — this cache is valid for the application lifetime.
    /// </summary>
    /// <remarks>
    /// Key: queryType
    /// Value: true if exception handlers exist, false otherwise
    /// Thread-safe via <see cref="ConcurrentDictionary{TKey, TValue}"/>.
    /// Populated on cold path (first dispatch per query type) via <see cref="ExceptionHandlerResolver"/>.
    /// </remarks>
    ...
    [UnconditionalSuppressMessage("Trimming", "IL2026",
        Justification = "Tier 2 path: NexumPipelineRegistry and its methods are generated by Nexum.SourceGenerators which emits explicit type registrations — types are preserved by the generator.")]
    [UnconditionalSuppressMessage("AOT", "IL3050",
        Justification = "Tier 2 path: MethodInfo.CreateDelegate on NexumPipelineRegistry methods — types are statically known to the Source Generator that produces the registry.")]
    [UnconditionalSuppressMessage("Trimming", "IL2074",
        Justification = "Tier 2 path: NexumPipelineRegistry methods are generated by Source Generators — types statically preserved.")]
    ...
    private async ValueTask<TResult> DispatchTier2ColdAsync<TResult>(
        IQuery<TResult> query,
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.Interfaces)] Type queryType,
        CancellationToken ct)
    {
        try
        {
    ...
    /// Avoids async state machine allocation on synchronously completing handlers.
    /// Implements three sub-paths for optimal performance:
    /// <list type="bullet">
    /// <item>Zero behaviors + zero exception handlers → direct handler call, no try/catch (zero-alloc)</item>
    /// <item>Zero behaviors + exception handlers → direct handler call with exception handling</item>
    /// <item>With behaviors → full pipeline with exception handling</item>
    /// </list>
    /// </summary>
    [UnconditionalSuppressMessage("Trimming", "IL2072",
        Justification = "Runtime-only path; handler types preserved by DI registrations. SG Tier 2/3 path eliminates reflection.")]
        ...
        CancellationToken ct)
    {
        ...
        // Sub-path 1: Zero behaviors + zero exception handlers (zero-alloc, no try/catch)
        // The wrapper's s_hasBehaviors=false means no DI lookup for behaviors.
        // The exception handlers cache=false means no try/catch needed.
        if (s_hasQueryExceptionHandlersCache.TryGetValue(queryType, out bool hasExHandlers) && !hasExHandlers)
        {
            return wrapper.HandleAsync(query, _serviceProvider, _options, ct);
        }

        // Sub-path 2 & 3: Has exception handlers (or not yet cached) — use try/catch wrapper
        try
        {
    ...
    /// <summary>
    /// Cold path for runtime query dispatch: resolves handler type, caches wrapper, populates exception
    /// handlers cache, then dispatches.
    /// </summary>
    private async ValueTask<TResult> DispatchRuntimeColdAsync<TResult>(
        ...
        CancellationToken ct)
    {
        // Cache exception handler existence on cold path (once per query type, application lifetime)
        bool hasExHandlers = s_hasQueryExceptionHandlersCache.GetOrAdd(
            queryType,
            static (qType, resolver) => resolver.HasQueryExceptionHandlers(qType),
            _exceptionHandlerResolver);

        QueryHandlerWrapper<TResult> wrapper = GetOrCreateQueryWrapper<TResult>(queryType);

        if (!hasExHandlers)
        {
            // No exception handlers — direct dispatch, no try/catch
            return await wrapper.HandleAsync(query, _serviceProvider, _options, ct)
                .ConfigureAwait(false);
        }
    ...
    [UnconditionalSuppressMessage("Trimming", "IL2055",
        Justification = "queryType is annotated with PublicConstructors|Interfaces; handlerType args are concrete closed generic types resolved at runtime from DI — trimmer cannot verify statically.")]
    [UnconditionalSuppressMessage("Trimming", "IL2062",
        Justification = "Generic arguments extracted from a closed IQueryHandler<,> type; handler implementation types are preserved by DI registrations.")]
    [UnconditionalSuppressMessage("AOT", "IL3050",
        Justification = "Runtime-only path; handler types preserved by DI registrations. SG Tier 2/3 path eliminates reflection.")]
    private QueryHandlerWrapper<TResult> GetOrCreateQueryWrapper<TResult>(
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.Interfaces)] Type queryType)
    {
        if (s_queryWrapperCache.TryGetValue(queryType, out object? cachedWrapper))
        {
    ...
    [UnconditionalSuppressMessage("Trimming", "IL2055",
        Justification = "queryType is annotated with PublicConstructors|Interfaces; handlerType args are concrete closed generic types resolved at runtime from DI — trimmer cannot verify statically.")]
    [UnconditionalSuppressMessage("Trimming", "IL2062",
        Justification = "Generic arguments extracted from a closed IStreamQueryHandler<,> type; handler implementation types are preserved by DI registrations.")]
    [UnconditionalSuppressMessage("AOT", "IL3050",
        Justification = "Runtime-only path; handler types preserved by DI registrations. SG Tier 2/3 path eliminates reflection.")]
    private StreamQueryHandlerWrapper<TResult> GetOrCreateStreamWrapper<TResult>(
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.Interfaces)] Type queryType)
    {
    ...
    internal static void ResetForTesting()
    {
        ...
        ExceptionHandlerResolver.ResetForTesting();
    }
}
