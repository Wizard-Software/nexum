...
public sealed class CommandDispatcher : ICommandDispatcher, IInterceptableDispatcher
{
    ...
    /// <summary>
    /// Cache indicating whether any command exception handlers are registered for a given command type.
    ...
    /// Thread-safe via <see cref="ConcurrentDictionary{TKey, TValue}"/>.
    /// Populated on cold path (first dispatch per command type) via <see cref="ExceptionHandlerResolver"/>.
    /// </remarks>
    private static readonly ConcurrentDictionary<Type, bool> s_hasExceptionHandlersCache = new();
    ...
    private async ValueTask<TResult> DispatchTier2ColdAsync<TResult>(
        ICommand<TResult> command,
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.Interfaces)] Type commandType,
        CancellationToken ct)
    {
        try
        {
        ...
        CancellationToken ct)
    {
        ...
        if (s_hasExceptionHandlersCache.TryGetValue(commandType, out bool hasExHandlers) && !hasExHandlers)
        {
            return wrapper.HandleAsync(command, _serviceProvider, _options, ct);
        }
    ...
    /// <summary>
    /// Cold path for runtime dispatch: resolves handler type, caches wrapper, populates exception
    ...
    private async ValueTask<TResult> DispatchRuntimeColdAsync<TResult>(
        ...
        CancellationToken ct)
    {
        // Cache exception handler existence on cold path (once per command type, application lifetime)
        bool hasExHandlers = s_hasExceptionHandlersCache.GetOrAdd(
            commandType,
            static (cmdType, resolver) => resolver.HasCommandExceptionHandlers(cmdType),
    ...
    [UnconditionalSuppressMessage("Trimming", "IL2055",
        Justification = "commandType is annotated with PublicConstructors|Interfaces; handlerType args are concrete closed generic types resolved at runtime from DI — trimmer cannot verify statically.")]
    [UnconditionalSuppressMessage("Trimming", "IL2062",
        Justification = "Generic arguments extracted from a closed ICommandHandler<,> type; handler implementation types are preserved by DI registrations.")]
    [UnconditionalSuppressMessage("AOT", "IL3050",
        Justification = "Runtime-only path; handler types preserved by DI registrations. SG Tier 2/3 path eliminates reflection.")]
    private CommandHandlerWrapper<TResult> GetOrCreateWrapper<TResult>(
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.Interfaces)] Type commandType)
    {
        if (s_wrapperCache.TryGetValue(commandType, out object? cachedWrapper))
        {
    ...
    internal static void ResetForTesting()
    {
        ...
        s_hasExceptionHandlersCache.Clear();
